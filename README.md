...
プレイヤーが最初に見る画面です。「プレイ開始」を押すとPython側のカメラ送信プログラムが自動起動し、MediaPipeがカメラを検出するまでの間は「カメラに接続中...」という待機画面を表示して、フルーツがまだ降ってこないようにゲームを一時停止しています。

| スタート画面 | 接続待機画面 |
| :---: | :---: |
| ![Start Screen](img/start_screen.png) | ![Waiting Screen](img/waiting_screen.png) |

### ゲームプレイ画面

左側がPython（MediaPipe）側のデバッグ映像、右側がUnityのゲーム画面です。手の21ランドマークを検出し、指先の軌跡でフルーツを切ると、スコア加算とコンボ表示（`COMBO x3!`）が発生します。

![Gameplay](img/gameplay.png)

### 画面遷移図

スタート画面からゲームオーバー画面までの遷移をまとめた図です。ライフが0になるとゲームオーバー画面へ遷移し、`R`キーまたはクリックでゲームプレイ画面へ戻ります。

![Screen Flow Diagram](img/screen_flow_diagram.png)

### ゲームロジックのフローチャート

`①手の認識`（カメラ→MediaPipe→UDP送信）と`②Unity/ゲームループ`（爆弾判定・スライス・スコア加算・ゲームオーバー）の処理の流れを示した図です。実際の`FruitNinjaGameController.cs`のロジック（爆弾を切るとライフ-1、ライフ0でゲームオーバー）と対応しています。

![Game Loop Flowchart](img/game_loop_flowchart.png)
